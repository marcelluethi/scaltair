...
object Test:
  def main(args: Array[String]): Unit =
    ...
    val dataFrames = for i <- 0 until 10 yield
      ...
      val df = DataFrame.fromColumns(
        Seq(
          ...
          DataFrame.Column
            .ofNominals(Seq.fill(xs.length)(s"function $i"), name = "series")
        )
      )
      print(df)
      df
    ...
    val combinedDataFrameForSeries =
      dataFrames.reduce((acc, x) => acc.concat(x))

    // we use the new data frame to plot the series
    combinedDataFrameForSeries.plot
      .linePlot(
        x = "x",
        y = "y",
        series = "series",
        title = "a series of linear functions"
      )
      .show()
